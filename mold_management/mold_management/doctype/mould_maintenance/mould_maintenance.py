...
# 		from mold_management.mold_management.doctype.mould_maintenance_task.mould_maintenance_task import MouldMaintenanceTask
...
# 	def validate(self):
# 		for task in self.get("mould_maintenance_tasks"):
...
# 		mold = frappe.get_doc("Mould", self.mould_name)
...
class MouldMaintenance(Document):
	...
	def validate(self):
		for task in self.get("mould_maintenance_tasks"):
			...
			# ✅ CHANGED: REMOVED mandatory Assign To validation
			# ❌ OLD CODE REMOVED:
			# if not task.assign_to and self.docstatus == 0:
			#     throw(_("Row #{}: Please asign task to a member.").format(task.idx))
	...
	def on_update(self):
		for task in self.get("mould_maintenance_tasks"):
			...
			# ✅ CHANGED: Safe check before assigning task
			if task.assign_to:
				assign_tasks(self.name, task.assign_to, task.maintenance_task, task.next_due_date)
...
def assign_tasks(mould_maintenance_name, assign_to_member, maintenance_task, next_due_date):
	...
	# ✅ CHANGED: Prevent crash if no user is assigned
