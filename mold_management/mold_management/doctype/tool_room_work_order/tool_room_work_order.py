...
class ToolRoomWorkOrder(Document):
	...
	if TYPE_CHECKING:
		...
		operations: DF.Table[ToolRoomWorkOrderOperation]
		...
		qty: DF.Float
		required_items: DF.Table[ToolRoomWorkOrderItem]
		...
		transfer_material_against: DF.Literal["", "Tool Room Work Order", "Job Card"]
	...
	def validate_work_order_against_so(self):
		...
		ordered_qty_against_so = frappe.db.sql(
			"""select sum(qty) from `tabTool Room Work Order`
	...
	def update_status(self, status=None):
		"""Update status of Tool Room Work Order if unknown"""
	...
	def get_status(self, status=None):
		...
		if self.docstatus == 0:
			...
			if status != "Stopped":
				...
				precision = frappe.get_precision("Tool Room Work Order", "produced_qty")
	...
	def update_tool_room_work_order_qty(self):
		...
		for purpose, fieldname in (
			...
			if qty > completed_qty:
				frappe.throw(
					_("{0} ({1}) cannot be greater than planned quantity ({2}) in Tool Room Work Order {3}").format(
			...
			from mold_management.selling.doctype.sales_order.sales_order import update_produced_qty_in_so_item
	...
	def validate_cancel(self):
		if self.status == "Stopped":
			frappe.throw(_("Stopped Tool Room Work Order cannot be cancelled, Unstop it first to cancel"))

		# Check whether any stock entry exists against this Work Order
		stock_entry = frappe.db.sql(
			"""select name from `tabStock Entry`
			where tool_room_work_order = %s and docstatus = 1""",
			self.name,
		)
		if stock_entry:
			frappe.throw(
	...
	def set_produced_qty_for_sub_assembly_item(self):
		table = frappe.qb.DocType("Tool Room Work Order")
	...
	def update_work_order_qty_in_so(self):
		...
		qty = frappe.db.sql(
			...
			`tabTool Room Work Order` where sales_order = %s and docstatus = 1 and {cond}
			""",
	...
	def set_work_order_operations(self):
		"""Fetch operations from BOM and set in 'Tool Room Work Order'"""
	...
	def validate_production_item(self):
		if frappe.get_cached_value("Item", self.production_item, "has_variants"):
			frappe.throw(_("Tool Room Work Order cannot be raised against a Item Template"), ItemHasVariantError)
	...
	def validate_transfer_against(self):
		...
		if not self.operations:
			self.transfer_material_against = "Tool Room Work Order"
	...
	def update_consumed_qty_for_required_items(self):
		...
		against a Tool Room Work Order for each stock item
		"""
...
def make_work_order(bom_no, item, qty=0, project=None, variant_items=None, use_multi_level_bom=None):
	if not frappe.has_permission("Tool Room Work Order", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	item_details = get_item_details(item, project)

	wo_doc = frappe.new_doc("Tool Room Work Order")
...
def set_work_order_ops(name):
	po = frappe.get_doc("Tool Room Work Order", name)
...
def make_stock_entry(work_order_id, purpose, qty=None, target_warehouse=None):
	work_order = frappe.get_doc("Tool Room Work Order", work_order_id)
...
def stop_unstop(work_order, status):
	...
	pro_order = frappe.get_doc("Tool Room Work Order", work_order)

	if pro_order.status == "Closed":
		frappe.throw(_("Closed Tool Room Work Order can not be stopped or Re-opened"))

	pro_order.update_status(status)
	pro_order.update_planned_qty()
	frappe.msgprint(_("Tool Room Work Order has been {0}").format(status))
...
def make_job_card(work_order, operations):
	...
	work_order = frappe.get_doc("Tool Room Work Order", work_order)
...
def close_work_order(work_order, status):
	...
	if work_order.get("operations"):
		...
		if job_cards:
			...
			frappe.throw(
				_("Can not close Tool Room Work Order. Since {0} Job Cards are in Work In Progress state.").format(
					job_cards
				)
			)
...
def create_pick_list(source_name, target_doc=None, for_qty=None):
	...
	max_finished_goods_qty = frappe.db.get_value("Tool Room Work Order", source_name, "qty")
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Tool Room Work Order": {"doctype": "Pick List", "validation": {"docstatus": ["=", 1]}},
			"Tool Room Work Order Item": {
...
def get_reserved_qty_for_production(
	...
	wo = frappe.qb.DocType("Tool Room Work Order")
	wo_item = frappe.qb.DocType("Tool Room Work Order Item")
...
def make_stock_return_entry(work_order):
	from mold_management.stock.doctype.stock_entry.stock_entry import get_available_materials

	non_consumed_items = get_available_materials(work_order)
	if not non_consumed_items:
		return

	wo_doc = frappe.get_cached_doc("Tool Room Work Order", work_order)
