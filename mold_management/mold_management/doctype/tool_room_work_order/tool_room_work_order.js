...
frappe.ui.form.on("Tool Room Work Order", {
	setup: function (frm) {
		...
		frm.set_query("operation", "required_items", function () {
			return {
				query: "mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.get_bom_operations",
				filters: {
		...
		frm.set_query("sales_order", function () {
			if (frm.doc.production_item) {
				return {
					query: "mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.query_sales_order",
					filters: {
		...
		if (frm.doc.allow_alternative_item && frm.doc.docstatus === 0 && has_alternative) {
			frm.add_custom_button(__("Alternate Item"), () => {
				erpnext.utils.select_alternate_items({
					...
					child_doctype: "Tool Room Work Order Item",
	...
	create_stock_return_entry: function (frm) {
		frappe.call({
			method: "mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.make_stock_return_entry",
			args: {
	...
	make_job_card: function (frm) {
		...
		const dialog = frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.make_job_card",
					freeze: true,
					args: {
						work_order: frm.doc.name,
	...
	make_disassembly_order(frm) {
		erpnext.work_order
			...
			.then((data) => {
				return frappe.xcall("mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.make_stock_entry", {
	...
	production_item: function (frm) {
		if (frm.doc.production_item) {
			frappe.call({
				method: "mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.get_item_details",
				args: {
...
frappe.ui.form.on("Tool Room Work Order Item", {
...
erpnext.work_order = {
	set_custom_buttons: function (frm) {
		...
		if (doc.docstatus === 1 && doc.status !== "Closed") {
			frm.add_custom_button(
				__("Close"),
				function () {
					frappe.confirm(__("Once the Tool Room Work Order is Closed. It can't be resumed."), () => {
	...
	create_pick_list: function (frm, purpose = "Material Transfer for Manufacture") {
		this.show_prompt_for_qty_input(frm, purpose)
			.then((data) => {
				return frappe.xcall("mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.create_pick_list", {
	...
	make_consumption_se: function (frm, backflush_raw_materials_based_on) {
		...
		frappe.call({
			method: "mold_management.mold_management.doctype.tool_room_work_order.tool_room_work_order.make_stock_entry",
			args: {
				work_order_id: frm.doc.name,
...
frappe.tour["Tool Room Work Order"] = [
	{
