...
from mold_management.selling.doctype.sales_order.test_sales_order import make_sales_order
from mold_management.stock.doctype.item.test_item import create_item, make_item
from mold_management.stock.doctype.serial_and_batch_bundle.test_serial_and_batch_bundle import (
	get_batch_from_bundle,
	get_serial_nos_from_bundle,
	make_serial_batch_bundle,
)
from mold_management.stock.doctype.serial_no.serial_no import get_serial_nos
from mold_management.stock.doctype.stock_entry import test_stock_entry
from mold_management.stock.doctype.warehouse.test_warehouse import create_warehouse
from mold_management.stock.utils import get_bin
...
class TestWorkOrder(FrappeTestCase):
	...
	def check_planned_qty(self):
		...
		self.assertEqual(frappe.db.get_value("Tool Room Work Order", wo_order.name, "produced_qty"), 4)
	...
	def test_scrap_material_qty(self):
		...
		wo_order_details = frappe.db.get_value(
			"Tool Room Work Order", wo_order.name, ["scrap_warehouse", "qty", "produced_qty", "bom_no"], as_dict=1
		)
	...
	def test_wo_completion_with_pl_bom(self):
		from mold_management.mold_management.doctype.bom.test_bom import (
	...
	def test_auto_batch_creation(self):
		from mold_management.mold_management.doctype.bom.test_bom import create_nested_bom
	...
	def test_get_rm_cost_from_consumption_entry(self):
		from mold_management.stock.doctype.stock_entry.test_stock_entry import (
	...
	def test_disassembly_order_with_qty_behavior(self):
		...
		self.assertEqual(
			wo.disassembled_qty,
			disassemble_qty,
			f"Tool Room Work Order disassembled_qty mismatch: expected {disassemble_qty}, got {wo.disassembled_qty}",
		)
...
def prepare_data_for_workstation_type_check():
	from mold_management.mold_management.doctype.operation.test_operation import make_operation
	from mold_management.mold_management.doctype.workstation.test_workstation import make_workstation
	from mold_management.mold_management.doctype.workstation_type.test_workstation_type import (
...
def make_wo_order_test_record(**args):
	...
	wo_order = frappe.new_doc("Tool Room Work Order")
	...
	wo_order.transfer_material_against = args.transfer_material_against or "Tool Room Work Order"
...
test_records = frappe.get_test_records("Tool Room Work Order")
