...
class MouldDepreciationSchedule(Document):
	...
	if TYPE_CHECKING:
		...
		from mold_management.doctype.mould_depreciation_schedule.mould_depreciation_schedule import (
	...
	def before_save(self):
		if not self.finance_book_id:
			self.prepare_draft_mould_depr_schedule_data_from_mould_name_and_fb_name(
				self.mould, self.finance_book
			)
		self.update_shift_depr_schedule()

	def validate(self):
		self.validate_another_mould_depr_schedule_does_not_exist()

	def validate_another_mould_depr_schedule_does_not_exist(self):
		finance_book_filter = ["finance_book", "is", "not set"]
		if self.finance_book:
			finance_book_filter = ["finance_book", "=", self.finance_book]

		mould_depr_schedule = frappe.db.exists(
			"Mould Depreciation Schedule",
			[
				["mould", "=", self.mould],
				finance_book_filter,
				["docstatus", "<", 2],
		...
		if mould_depr_schedule and mould_depr_schedule != self.name:
			if self.finance_book:
				frappe.throw(
					_(
						"Mould Depreciation Schedule {0} for Mould {1} and Finance Book {2} already exists."
					).format(mould_depr_schedule, self.mould, self.finance_book)
				)
			else:
				frappe.throw(
					_("Mould Depreciation Schedule {0} for Mould {1} already exists.").format(
						mould_depr_schedule, self.mould
					)
				)
	...
	def validate_mould(self):
		mould = frappe.get_doc("Mould", self.mould)
		if not mould.calculate_depreciation:
			frappe.throw(
				_("Mould {0} is not set to calculate depreciation.").format(
					get_link_to_form("Mould", self.mould)
				)
			)
		if mould.docstatus != 1:
			frappe.throw(
				_("Mould {0} is not submitted. Please submit the mould before proceeding.").format(
					get_link_to_form("Mould", self.mould)
				)
			)
	...
	def update_shift_depr_schedule(self):
		...
		mould_doc = frappe.get_doc("Mould", self.mould)
		fb_row = mould_doc.finance_books[self.finance_book_id - 1]

		self.make_depr_schedule(mould_doc, fb_row)
		self.set_accumulated_depreciation(mould_doc, fb_row)

	def prepare_draft_mould_depr_schedule_data_from_mould_name_and_fb_name(self, mould_name, fb_name):
		mould_doc = frappe.get_doc("Mould", mould_name)

		finance_book_filter = ["finance_book", "is", "not set"]
		if fb_name:
			finance_book_filter = ["finance_book", "=", fb_name]

		mould_finance_book_name = frappe.db.get_value(
			doctype="Mould Finance Book",
			filters=[["parent", "=", mould_name], finance_book_filter],
		)
		mould_finance_book_doc = frappe.get_doc("Mould Finance Book", mould_finance_book_name)

		self.prepare_draft_mould_depr_schedule_data(mould_doc, mould_finance_book_doc)

	def prepare_draft_mould_depr_schedule_data(
		self,
		mould_doc,
		row,
		date_of_disposal=None,
		date_of_return=None,
		update_mould_finance_book_row=True,
	):
		have_mould_details_been_modified = self.have_mould_details_been_modified(mould_doc)
		...
		self.set_draft_mould_depr_schedule_details(mould_doc, row)

		if self.should_prepare_depreciation_schedule(
			have_mould_details_been_modified, not_manual_depr_or_have_manual_depr_details_been_modified
		):
			self.make_depr_schedule(mould_doc, row, date_of_disposal, update_mould_finance_book_row)
			self.set_accumulated_depreciation(mould_doc, row, date_of_disposal, date_of_return)

	def have_mould_details_been_modified(self, mould_doc):
		return (
			mould_doc.gross_purchase_amount != self.gross_purchase_amount
			or mould_doc.opening_accumulated_depreciation != self.opening_accumulated_depreciation
			or mould_doc.opening_number_of_booked_depreciations != self.opening_number_of_booked_depreciations
		)
	...
	def should_prepare_depreciation_schedule(
		self, have_mould_details_been_modified, not_manual_depr_or_have_manual_depr_details_been_modified
	):
		if not self.get("depreciation_schedule"):
			return True

		old_mould_depr_schedule_doc = self.get_doc_before_save()

		if self.docstatus != 0 and not old_mould_depr_schedule_doc:
			return True

		if have_mould_details_been_modified or not_manual_depr_or_have_manual_depr_details_been_modified:
			return True

		return False

	def set_draft_mould_depr_schedule_details(self, mould_doc, row):
		self.mould = mould_doc.name
		self.finance_book = row.finance_book
		self.finance_book_id = row.idx
		self.opening_accumulated_depreciation = mould_doc.opening_accumulated_depreciation or 0
		self.opening_number_of_booked_depreciations = mould_doc.opening_number_of_booked_depreciations or 0
		self.gross_purchase_amount = mould_doc.gross_purchase_amount
	...
	def make_depr_schedule(
		...
		if not mould_doc.available_for_use_date:
			return

		start = self.clear_depr_schedule()

		self._make_depr_schedule(
			mould_doc, row, start, date_of_disposal, update_mould_finance_book_row, value_after_depreciation
		)
	...
	def _make_depr_schedule(
		...
		start,
		date_of_disposal,
		update_mould_finance_book_row,
		value_after_depreciation,
	):
		mould_doc.validate_mould_finance_books(row)
		if (
			not value_after_depreciation
			and not mould_doc.flags.decrease_in_mould_value_due_to_value_adjustment
		):
			value_after_depreciation = _get_value_after_depreciation_for_making_schedule(mould_doc, row)
		row.value_after_depreciation = value_after_depreciation

		if update_mould_finance_book_row:
			row.db_update()
		...
		has_pro_rata = _check_is_pro_rata(mould_doc, row)
		if has_pro_rata:
		...
		for n in range(start, final_number_of_depreciations):
			...
			if date_of_disposal and getdate(schedule_date) >= getdate(date_of_disposal):
				from_date = add_months(
					...
					(mould_doc.opening_number_of_booked_depreciations * row.frequency_of_depreciation),
				)
				if is_last_day_of_the_month(getdate(mould_doc.available_for_use_date)):
				...
				depreciation_amount = flt(depreciation_amount, mould_doc.precision("gross_purchase_amount"))
			...
			if (
				...
				if flt(depreciation_amount, mould_doc.precision("gross_purchase_amount")) <= 0:
					frappe.throw(
						_(
							"Gross Purchase Amount Too Low: {0} cannot be depreciated over {1} cycles with a frequency of {2} depreciations."
						).format(
							frappe.bold(mould_doc.gross_purchase_amount),
			...
			# For last row
				...
				if not mould_doc.flags.increase_in_mould_life:
					# In case of increase_in_mould_life, the mould.to_date is already set on mould_repair submission
					mould_doc.to_date = add_months(
						mould_doc.available_for_use_date,
						(n + self.opening_number_of_booked_depreciations)
						* cint(row.frequency_of_depreciation),
					)
					if is_last_day_of_the_month(getdate(mould_doc.available_for_use_date)):
						mould_doc.to_date = get_last_day(mould_doc.to_date)
			...
			value_after_depreciation = flt(
				...
				mould_doc.precision("gross_purchase_amount"),
			)
	...
	def add_depr_schedule_row(self, schedule_date, depreciation_amount, schedule_idx):
		if self.shift_based:
			shift = (
				...
				else frappe.get_cached_value("Mould Shift Factor", {"default": 1}, "shift_name")
			)
		else:
			shift = None
	...
	def set_accumulated_depreciation(
		...
		for i, d in enumerate(self.get("depreciation_schedule")):
			...
			if not accumulated_depreciation:
				if i > 0 and (
					mould_doc.flags.decrease_in_mould_value_due_to_value_adjustment
					or mould_doc.flags.increase_in_mould_value_due_to_repair
				):
					...
					accumulated_depreciation = flt(
						...
						mould_doc.precision("opening_accumulated_depreciation"),
					)
...
def _get_value_after_depreciation_for_making_schedule(mould_doc, fb_row):
	if mould_doc.docstatus == 1 and fb_row.value_after_depreciation:
		value_after_depreciation = flt(fb_row.value_after_depreciation)
	else:
		value_after_depreciation = flt(mould_doc.gross_purchase_amount) - flt(
			mould_doc.opening_accumulated_depreciation
		)
...
def _check_is_pro_rata(mould_doc, row, wdv_or_dd_non_yearly=False):
	has_pro_rata = False
	...
	if row.depreciation_method in ("Straight Line", "Manual"):
		prev_depreciation_start_date = get_last_day(
			add_months(
				row.depreciation_start_date,
				(row.frequency_of_depreciation * -1) * mould_doc.opening_number_of_booked_depreciations,
			)
		)
		from_date = mould_doc.available_for_use_date
		...
		from_date = _get_modified_available_for_use_date(mould_doc, row, wdv_or_dd_non_yearly=False)
		days = date_diff(row.depreciation_start_date, from_date) + 1
		total_days = get_total_days(row.depreciation_start_date, row.frequency_of_depreciation)
	if days <= 0:
		frappe.throw(
			_(
				"""Error: This mould already has {0} depreciation periods booked.
				The `depreciation start` date must be at least {1} periods after the `available for use` date.
				Please correct the dates accordingly."""
			).format(
				mould_doc.opening_number_of_booked_depreciations,
				mould_doc.opening_number_of_booked_depreciations,
			)
		)
	if days < total_days:
...
def _get_modified_available_for_use_date(mould_doc, row, wdv_or_dd_non_yearly=False):
	"""
	if Mould has opening booked depreciations = 9,
	available for use date = 17-07-2023,
	depreciation start date = 30-04-2024
	then from date should be 01-04-2024
	"""
	if mould_doc.opening_number_of_booked_depreciations > 0:
		from_date = add_months(
			mould_doc.available_for_use_date,
			(mould_doc.opening_number_of_booked_depreciations * row.frequency_of_depreciation) - 1,
		)
		...
		return mould_doc.available_for_use_date
...
def get_depreciation_amount(
	...
	if fb_row.depreciation_method in ("Straight Line", "Manual"):
		return get_straight_line_or_manual_depr_amount(
			mould_depr_schedule, mould, fb_row, schedule_idx, number_of_pending_depreciations
		), None
	else:
...
def get_straight_line_or_manual_depr_amount(
	mould_depr_schedule, mould, row, schedule_idx, number_of_pending_depreciations
):
	if row.shift_based:
		return get_shift_depr_amount(mould_depr_schedule, mould, row, schedule_idx)

	# if the Depreciation Schedule is being modified after Asset Repair due to increase in asset life and value
	if mould.flags.increase_in_mould_life:
		return (flt(row.value_after_depreciation) - flt(row.expected_value_after_useful_life)) / (
			date_diff(mould.to_date, mould.available_for_use_date) / 365
		)
	# if the Depreciation Schedule is being modified after Asset Repair due to increase in asset value
	elif mould.flags.increase_in_mould_value_due_to_repair:
		return (flt(row.value_after_depreciation) - flt(row.expected_value_after_useful_life)) / flt(
			number_of_pending_depreciations
		)
	# if the Depreciation Schedule is being modified after Asset Value Adjustment due to decrease in asset value
	elif mould.flags.decrease_in_mould_value_due_to_value_adjustment:
	...
	# if the Depreciation Schedule is being prepared for the first time
		...
		if row.daily_prorata_based:
			amount = flt(mould.gross_purchase_amount) - flt(row.expected_value_after_useful_life)
			return get_daily_prorata_based_straight_line_depr(
				mould, row, schedule_idx, number_of_pending_depreciations, amount
			)
		else:
			depreciation_amount = (
				flt(mould.gross_purchase_amount) - flt(row.expected_value_after_useful_life)
...
def get_daily_depr_amount(mould, row, schedule_idx, amount):
				...
				add_days(
					get_last_day(
						add_months(
							...
							(
								row.frequency_of_depreciation
								* (mould.opening_number_of_booked_depreciations + 1)
							)
							* -1,
						),
					),
...
def get_shift_depr_amount(mould_depr_schedule, mould, row, schedule_idx):
	if mould_depr_schedule.get("__islocal") and not mould.flags.shift_allocation:
		return (
			flt(mould.gross_purchase_amount)
			- flt(mould.opening_accumulated_depreciation)
			- flt(row.expected_value_after_useful_life)
		) / flt(row.total_number_of_depreciations - mould.opening_number_of_booked_depreciations)

	mould_shift_factors_map = get_mould_shift_factors_map()
	shift = (
		mould_depr_schedule.schedules_before_clearing[schedule_idx].shift
		if len(mould_depr_schedule.schedules_before_clearing) > schedule_idx
		else None
	)
	shift_factor = mould_shift_factors_map.get(shift) if shift else 0

	shift_factors_sum = sum(
		flt(mould_shift_factors_map.get(schedule.shift))
		for schedule in mould_depr_schedule.schedules_before_clearing
	)
...
def get_mould_shift_factors_map():
	return dict(frappe.db.get_all("Mould Shift Factor", ["shift_name", "shift_factor"], as_list=True))
